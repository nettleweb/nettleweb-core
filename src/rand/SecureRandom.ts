...
export default class SecureRandom extends AbstractRandom<number[]> implements NettleWeb.SecureRandom {
	constructor(seed?: number | number[] | nul) {
		super(typeof (seed ||= seedgen()) === "number" ? seed = [seed] : seed);
		this.setSeed(seed);
	}

	#m: number[] = new Array(256); // internal memory
	#r: number[] = new Array(256); // result array

	#acc: number = 0; // accumulator
	#brs: number = 0; // last result
	#cnt: number = 0; // counter
	#gnt: number = 0; // generation counter
	...
	setSeed(seed: number | number[]) {
		...
		for (let i = 0; i < 256; i += 8) {
			...
			a = this.#add(a, r[i + 0]); b = this.#add(b, r[i + 1]);
			c = this.#add(c, r[i + 2]); d = this.#add(d, r[i + 3]);
			e = this.#add(e, r[i + 4]); f = this.#add(f, r[i + 5]);
			g = this.#add(g, r[i + 6]); h = this.#add(h, r[i + 7]);
		...
		for (let i = 0; i < 256; i += 8) {
			a = this.#add(a, m[i + 0]); b = this.#add(b, m[i + 1]);
			c = this.#add(c, m[i + 2]); d = this.#add(d, m[i + 3]);
			e = this.#add(e, m[i + 4]); f = this.#add(f, m[i + 5]);
			g = this.#add(g, m[i + 6]); h = this.#add(h, m[i + 7]);
